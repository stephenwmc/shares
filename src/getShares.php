...
function getScore($inScore)
{
    ...
    if ($inScore == $lastShare) {
        ...
        $getScore = 0;
        $shareDetail = ShareDetail::where("share_code","=",$inScore)->first();

        $getScore = $getScore + $shareDetail->score;
        
        $recommendations = Recommendation::where("share_code","=", $inScore)->groupBy("share_code")->get();
...
//        while ($recommendations->read()) {
            $getScore = $getScore + $recommendation->t_score;
            $shareDetail->rec_score = $recommentdaion->t_score;
            $shareDetail->save();
